...
def get_args():
    parser = argparse.ArgumentParser(
        description="This script downloads the LAP dataset "
...
def reporthook(count, block_size, total_size):
    ...
    sys.stdout.write(
        "\r{}%, {:6.2f}/{:6.2f}MB, {}KB/s, passed: {}s, remaining: {}s".format(
            percent,
...
def crop():
    ...
    if not detector_model_path.is_file():
        ...
        with (
            open(detector_model_bz2, "rb") as source,
            open(str(detector_model_path), "wb") as dest,
        ):
            dest.write(bz2.decompress(source.read()))

    detector = dlib.cnn_face_detection_model_v1(str(detector_model_path))

    for image_dir, crop_dir in [
        [train_image_dir, train_crop_dir],
        [validation_image_dir, validation_crop_dir],
    ]:
        for image_path in image_dir.glob("*.jpg"):
            ...
            if len(dets) != 1:
                print(
                    "{} faces were detected for {}".format(len(dets), image_path.name)
                )
                rects = [
                    [d.rect.left(), d.rect.right(), d.rect.top(), d.rect.bottom()]
                    for d in dets
                ]
                print(rects)
...
def extract():
    for zip_name in zip_names:
        ...
        if not zip_path.is_file():
            raise RuntimeError(
                "{} was not found. Please download the LAP dataset.".format(zip_name)
            )
